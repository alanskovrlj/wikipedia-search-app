...
function SearchBar({searchString, setSearchString, resultSet, responseTime, setShowModal}) {
  return (
    <div className='mb-10'>
        ...
        <div className='text-left text-xs pl-5 pt-2  md:w-1/2 m-auto text-slate-400'>{resultSet?.length == 0 ? "No results for '" + searchString +"'": "Showing " + resultSet?.length + " results"}</div>
        </div>
  )
}
